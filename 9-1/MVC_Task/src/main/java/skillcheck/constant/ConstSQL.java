...
//    private static final String SQL_INSERT = "INSERT INTO Employee (empid, passwoed, name, mail, programinglanguage, comment, createdate, updatedate, deleteflg) VALUES ('?', '?', '?', '?', '?', '?', '?', '?', '?');
    private static final String SQL_UPDATE = "UPDATE Employee SET passwoed = ?, name = ?, mail = ?, programinglanguage = ?, comment = ?, createdate = ?, updatedate = ?, deleteflg = ? WHERE emplod = ?";
    private static final String SQL_SELECT = "SELECT * FROM Employee WHERE Emplod = ?, Password = ?";


    // FIXME Step-2-1: [SQL-SELECT] 社員情報テーブルより、作成日時、更新日時、削除フラグを除いたカラムを取得するクエリを作成しなさい。
    /** 社員情報一覧取得用クエリ: 取得カラム + 取得元テーブル */
    public static final String SELECT_BASE = " SELECT empid, passwoed, name, mail, programinglanguage, comment FROM Employee ";
